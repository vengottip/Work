    ...
    public static class TopTitlesStatReduce extends Reducer<NullWritable, TextArrayWritable, Text, IntWritable> {
        ...
        public void reduce(NullWritable key, Iterable<TextArrayWritable> values, Context context) throws IOException, InterruptedException {
            ...
            while(iterator.hasNext()) {
                ...
                //System.err.println("Inside the TopTitlesStatReduce iterator set: key " + title+ " , value " + count  );
                cumNum += count;
                numRecords++;
                if(count < min) {
                    min = count;
            ...
            int quotient = cumNum / numRecords;
            mean = (int) Math.floor(quotient);
            sum = Integer.valueOf(cumNum);
            int squaredDiffSum = 0;

            iterator = pairTreeSet.iterator();
            while(iterator.hasNext()) {
                Pair pair = iterator.next();
                int count = (int)pair.first;
                //String title = pair.second.toString();
               
                int diffFromMean = count - mean;
                squaredDiffSum += (diffFromMean * diffFromMean);
                //System.err.println("Inside the TopTitlesStatReduce reduce iterator set: mean =  " + mean+ " , value = " + count + " diffFrommean =" +diffFromMean + " squaredDiffSum = " + squaredDiffSum );
            }
            int quotient1 = squaredDiffSum / numRecords;
            var =  (int) Math.floor(quotient1);  ;
            //var =  Math.floorDiv(squaredDiffSum, numRecords -1) ;
